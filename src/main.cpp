...
uint256 hashGenesisBlock("00000544496fbd72de6d6f83ba4444c8ca43bc4c94535d973c4f370a1387bd7f");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MKTCoins: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MKTCoins Signed Message:\n";
...
{
    // MKTCoins: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // MKTCoins: each block
static const int64 nTargetSpacing = 3 * 60; // MKTCoins: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MKTCoins: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, MKTCoins - DarkGravity, written by Evan Duffield - evan@MKTCoins.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MKTCoins: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MKTCoins: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "MKTCoins";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x8863f79ebf7a192018372cc1723ffcfa322d38837f2183eca5346b72e539568f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // MKTCoins: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MKTCoinsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MKTCoinsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MKTCoinsMiner(CWallet *pwallet)
{
    printf("MKTCoinsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("MKTCoins-miner");
    ...
    try { while (true) {
        ...
        printf("Running MKTCoinsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MKTCoinsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MKTCoinsMiner, pwallet));
}
